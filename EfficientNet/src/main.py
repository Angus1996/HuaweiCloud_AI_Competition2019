...
# from LabelSmoothingLoss import *
...
# parser.add_argument('-a', '--arch', metavar='ARCH', required=False,
#                     choices=model_names,
...
parser.add_argument('--wd', '--weight-decay', default=1e-4, type=float,
                    ...
                    dest='weight_decay')
parser.add_argument('-p', '--print_freq', default=2, type=int,
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.pretrained:
        ...
        if not mox.file.exists('../pre-trained_model/pytorch/efficientnet-b7-dcc49843.pth'):
            mox.file.copy('s3://cmhbucket/weights//efficientnet-b7-dcc49843.pth',
    ...
    criterion2 = LabelSmoothSoftmaxCE(lb_pos=0.9, lb_neg=0.005).cuda(args.gpu)
    # criterion = FocalLoss(class_num=54, gamma=2, size_average=True).cuda(args.gpu)
    ...
    train_dataset = datasets.ImageFolder(
        ...
        transforms.Compose([
            ...
            transforms.RandomAffine(degrees=180, translate=(0.1, 0.1), scale=(0.8, 1.2), shear=(20, 30), fillcolor=0),
    ...
    if args.eval_pth != '':
        ...
        validate(val_loader, model, criterion2, args)
        return
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        train(train_loader, model, criterion2, optimizer, epoch, args)

        # evaluate on validation set
        if (epoch + 1) % args.print_freq == 0:
            acc1 = validate(val_loader, model, criterion2, args)
...
def train(train_loader, model, criterion2, optimizer, epoch, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        # loss = label_smoothing(output, target, 0.15)
...
def validate(val_loader, model, criterion2, args):
    batch_time = AverageMeter('Time', ':6.3f')
    losses = AverageMeter('Loss', ':.4e')
    top1 = AverageMeter('Acc@1', ':6.2f')
    top5 = AverageMeter('Acc@5', ':6.2f')
