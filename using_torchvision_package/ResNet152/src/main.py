...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.pretrained:
        ...
        if not mox.file.exists('../pre-trained_model/pytorch/resnet152-b121ed2d.pth'):
            mox.file.copy('s3://resnet152/weights/resnet152-b121ed2d.pth',
                          '../pre-trained_model/pytorch/resnet152-b121ed2d.pth')
            print('copy pre-trained model from OBS to: %s success' %
                  (os.path.abspath('../pre-trained_model/pytorch/resnet152-b121ed2d.pth')))
        else:
    ...
    # criterion = LabelSmoothingLoss(0.1, 54, ignore_index=-1).cuda(args.gpu)
    ...
    train_dataset = datasets.ImageFolder(
        ...
        transforms.Compose([
            ...
            transforms.RandomAffine(degrees=30, translate=(0, 0.2), scale=(0.8, 1.2), shear=(10, 20), fillcolor=0),
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        # label smoothing
        # target = ((1.0-0.1)*target) + (1.0/54)
        target = target.cuda(args.gpu, non_blocking=True)

        # compute output
        output = model(images)
        # loss = criterion(output, target)
        loss = label_smoothing(output, target)
